...
`);
  ...
  static toggleAlert() {
    $(".alert")
      .alert()
      .show(); // Keep close.bs.alert event from removing from DOM
  }
      ...
      if ($("#name-input").val() && $("#add-description").val()) {
        ...
        if ($("#myDate") && $("#myDate").val()) {
          ...
          if (date) {
            ...
            Controller.renderNote(
              Calendar.all.notes[Calendar.all.notes.length - 1]
            );
  ...
  static RenderExtraFeatures() {
    ...
    let selectMenu = $('<select id= "features-menu"> </select>').appendTo(
      divContainer
    );
    let addFeature = $("<option>")
      .attr("value", "add a Feature")
      .attr("id", "add-Feature")
      .html("");
    let addDate = $("<option>")
      .attr("value", "Add-Date")
